...
<style>
    ...
    @media (max-width: 1024px) { .form-layout { grid-template-columns: 1fr; } .summary-card { position: static; } }
    @media (max-width: 768px) { .form-layout { margin: 16px 14px 24px; } .page-title-row { padding-left: 16px; padding-right: 16px; } .form-row-3 { grid-template-columns: 1fr; } }
</style>
    ...
    <div class="page-title-row">
        ...
        <a href="<?= site_url('kasir/penjualan') ?>" class="btn-back"><i class="fas fa-arrow-left"></i> Kembali</a>
    </div>

    <form action="<?= site_url('kasir/penjualan/simpan') ?>" method="post" id="formPenjualan">
    <div class="form-layout">
                ...
                <div class="form-card-body">
                    <div class="form-row-3">
                        <div class="form-group">
                            ...
                            <div class="input-wrap"><i class="fas fa-barcode input-icon"></i><input type="text" name="kode_transaksi" value="<?= $kode ?>" class="form-control readonly" readonly></div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-wrap"><i class="fas fa-calendar input-icon"></i><input type="text" class="form-control readonly" value="<?= date('d M Y H:i') ?>" readonly></div>
                        </div>
                        <div class="form-group">
                            <label class="form-label">Nama Pelanggan</label>
                            <div class="input-wrap"><i class="fas fa-user input-icon"></i><input type="text" name="nama_pelanggan" class="form-control" placeholder="Opsional (untuk nota)"></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="form-card">
                ...
                <div class="form-card-body">
                    <div class="items-header">
                        ...
                        <button type="button" class="btn-add-item" onclick="addItem()"><i class="fas fa-plus"></i> Tambah Item</button>
                    </div>
                    <div class="items-table-wrap">
                        <table class="items-table"><thead><tr><th style="width:35%;">Produk</th><th style="width:12%;">Stok</th><th style="width:12%;">Qty</th><th style="width:18%;">Harga Jual</th><th style="width:18%;">Subtotal</th><th style="width:5%;"></th></tr></thead>
                            <tbody id="itemsBody"><tr id="emptyRow"><td colspan="6" class="empty-items"><i class="fas fa-shopping-basket"></i>Keranjang kosong. Klik "+ Tambah Item" untuk mulai.</td></tr></tbody>
                        </table>
                    </div>
                </div>
            </div>
        ...
        <div>
            <div class="summary-card">
                <div class="summary-header"><h3><i class="fas fa-calculator" style="color:#1a56db; margin-right:6px;"></i>Ringkasan Pembayaran</h3></div>
                <div class="summary-body">
                    <div class="summary-row"><span class="label">Kode Transaksi</span><span class="value" style="font-family:'Courier New', monospace; font-size:12px; color:#1a56db;"><?= $kode ?></span></div>
                    ...
                    <button type="submit" class="btn-bayar" id="btnBayar" disabled><i class="fas fa-cash-register"></i> Bayar & Simpan Transaksi</button>
                    <div style="margin-top:10px; padding:10px; background:#eff6ff; border-radius:8px; font-size:11.5px; color:#1a56db; display:flex; gap:7px; align-items:flex-start;"><i class="fas fa-info-circle" style="margin-top:1px; flex-shrink:0;"></i><span>Stok produk akan otomatis berkurang setelah transaksi disimpan.</span></div>
                </div>
            </div>
        </div>
    </div>
...
<script>
    var produkData = <?= json_encode(array_map(function($p) {
        return ['id_product'=>$p['id_product'],'nama_produk'=>$p['nama_produk'],'satuan'=>$p['satuan'],'nama_kategori'=>$p['nama_kategori'],'stok'=>(int)$p['stok'],'harga_jual'=>(float)$p['harga_jual']];
    }, $produk)) ?>;
    var grandTotal = 0;
    function addItem() {
        var emptyRow = document.getElementById('emptyRow'); if (emptyRow) emptyRow.remove();
        var idx = Date.now();
        var options = produkData.map(function(p){return '<option value="'+p.id_product+'" data-stok="'+p.stok+'" data-harga="'+p.harga_jual+'" data-satuan="'+p.satuan+'">'+p.nama_produk+' ('+p.satuan+')</option>';}).join('');
        var row = document.createElement('tr'); row.id = 'item-row-'+idx;
        row.innerHTML = `<td><select name="id_product[]" class="item-select" onchange="onSelectProduk(${idx})" required><option value="" disabled selected>-- Pilih Produk --</option>${options}</select></td><td><span id="stok-${idx}" class="stok-hint" style="font-size:13px; color:#9ca3af;">—</span></td><td><input type="number" name="qty[]" id="qty-${idx}" class="item-input" placeholder="0" min="1" style="width:80px;" oninput="updateRow(${idx})" required></td><td><input type="number" name="harga_jual[]" id="harga-${idx}" class="item-input" placeholder="0" min="0" oninput="updateRow(${idx})" required></td><td><span id="subtotal-${idx}" class="item-subtotal" style="color:#9ca3af;">Rp 0</span></td><td><button type="button" class="btn-remove" onclick="removeItem(${idx})"><i class="fas fa-times"></i></button></td>`;
        document.getElementById('itemsBody').appendChild(row); updateSummary();
    }
    function onSelectProduk(idx) {
        var select=document.querySelector('#item-row-'+idx+' select'),opt=select.options[select.selectedIndex];
        var stok=parseInt(opt.getAttribute('data-stok'))||0,harga=parseFloat(opt.getAttribute('data-harga'))||0;
        var stokEl=document.getElementById('stok-'+idx);stokEl.textContent=stok+' '+(opt.getAttribute('data-satuan')||'');stokEl.className='stok-hint'+(stok<=0?' danger':stok<=10?' warning':'');
        document.getElementById('harga-'+idx).value=harga;document.getElementById('qty-'+idx).setAttribute('max',stok);updateRow(idx);
    }
    function removeItem(idx) {
        var row=document.getElementById('item-row-'+idx);if(row)row.remove();
        if(document.querySelectorAll('#itemsBody tr:not(#emptyRow)').length===0){var e=document.createElement('tr');e.id='emptyRow';e.innerHTML='<td colspan="6" class="empty-items"><i class="fas fa-shopping-basket"></i>Keranjang kosong.</td>';document.getElementById('itemsBody').appendChild(e);}
        updateSummary();
    }
    ...
    function updateKembalian() {
        ...
        var box=document.getElementById('kembalianBox'),valEl=document.getElementById('kembalianValue');
        if(bayar>0&&kembalian<0){box.className='kembalian-box kurang';valEl.textContent='Kurang Rp '+Math.abs(kembalian).toLocaleString('id-ID');}
        else{box.className='kembalian-box';valEl.textContent='Rp '+Math.max(0,kembalian).toLocaleString('id-ID');}
        var rows=document.querySelectorAll('#itemsBody tr:not(#emptyRow)'),stokOk=true;
        rows.forEach(function(row){var sel=row.querySelector('select'),qty=parseInt(row.querySelector('input[name="qty[]"]')?.value)||0;if(sel&&sel.value){var stok=parseInt(sel.options[sel.selectedIndex].getAttribute('data-stok'))||0;if(qty>stok)stokOk=false;}});
        document.getElementById('btnBayar').disabled=!(rows.length>0&&grandTotal>0&&bayar>=grandTotal&&stokOk);
    }
    document.getElementById('formPenjualan').addEventListener('submit', function(e) {
        var rows=document.querySelectorAll('#itemsBody tr:not(#emptyRow)');if(rows.length===0){e.preventDefault();alert('Keranjang masih kosong!');return;}
        var valid=true,stokOk=true;rows.forEach(function(row){var sel=row.querySelector('select'),qty=row.querySelector('input[name="qty[]"]'),harga=row.querySelector('input[name="harga_jual[]"]');if(!sel?.value||!qty?.value||!harga?.value)valid=false;if(sel&&sel.value){var stok=parseInt(sel.options[sel.selectedIndex].getAttribute('data-stok'))||0;if(parseInt(qty.value)>stok)stokOk=false;}});
        if(!valid){e.preventDefault();alert('Lengkapi semua data produk!');return;}if(!stokOk){e.preventDefault();alert('Qty melebihi stok!');return;}
        var bayar=parseFloat(document.getElementById('inputBayar').value)||0;if(bayar<grandTotal){e.preventDefault();alert('Bayar kurang dari total!');return;}
    });
</script>
</div>
